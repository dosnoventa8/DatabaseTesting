...
public class BorrowingDAO {
    ...
    public Borrowing create(Borrowing borrowing) throws SQLException {
        // MODIFIKASI UNTUK SOLUSI NO 4:
        // Cek apakah borrowDate diisi manual (biasanya untuk testing overdue)
        if (borrowing.getBorrowDate() != null) {
            // SQL query JIKA borrow_date disediakan manual
            String sql = "INSERT INTO borrowings (user_id, book_id, due_date, status, notes, borrow_date) " +
                    "VALUES (?, ?, ?, ?, ?, ?) " +
            ...
            try (Connection conn = DatabaseConfig.getConnection();
                ...
                // Set borrow_date manual
                pstmt.setTimestamp(6, borrowing.getBorrowDate());

                ResultSet rs = pstmt.executeQuery();
                if (rs.next()) {
                    // Set generated values
                    borrowing.setBorrowingId(rs.getInt("borrowing_id"));
                    // borrowDate tidak perlu di-set dari DB karena kita yang kirim
            ...
            // LOGIKA LAMA (NORMAL FLOW):
            // SQL query untuk insert borrowing record (borrow_date otomatis NOW() dari database)
